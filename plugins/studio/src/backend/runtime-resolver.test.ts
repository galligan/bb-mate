...
async function runtimeFixture() {
  const root = await mkdtemp(path.join(tmpdir(), "bb-plugin-studio-resolver-"));
  roots.push(root);
...
describe("packaged runtime resolver", () => {
  ...
  test("rejects symlinked and multiply-linked runtime artifacts", async () => {
    ...
    const outsideRoot = await mkdtemp(
      path.join(tmpdir(), "bb-plugin-studio-outside-"),
    );
    roots.push(outsideRoot);
    const outside = path.join(outsideRoot, "bb-plugin-studio-runtime");
  ...
  test("rejects a symlinked packaged runtime directory ancestor", async () => {
    ...
    const outsideRoot = await mkdtemp(
      path.join(tmpdir(), "bb-plugin-studio-runtime-dir-"),
    );
    roots.push(outsideRoot);
    await rename(
  ...
  test("rejects a symlinked loaded server and catches post-resolution drift", async () => {
    ...
    const outsideRoot = await mkdtemp(
      path.join(tmpdir(), "bb-plugin-studio-server-link-"),
    );
    roots.push(outsideRoot);
