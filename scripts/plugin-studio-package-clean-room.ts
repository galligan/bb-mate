...
import { RUNTIME_ARTIFACT_STAMP } from "../plugins/studio/src/generated/runtime-artifact-stamp.ts";
...
import { createPluginStudioRuntimeStamp } from "./plugin-studio-package-artifact.ts";
...
export async function runPluginStudioPackageCleanRoom(): Promise<void> {
  ...
  const normalProfileRoots = [
    ...
    path.join(normalHome, ".bb", "plugins", "studio"),
    path.join(normalHome, ".bb", "plugins", "cache", "npm", "bb-plugin-studio"),
    ...(process.env.BB_DATA_DIR && path.isAbsolute(process.env.BB_DATA_DIR)
      ? [
          path.join(process.env.BB_DATA_DIR, "plugins", "studio"),
          path.join(
    ...
    ...(process.env.XDG_DATA_HOME
      ? [path.join(process.env.XDG_DATA_HOME, "bb", "plugins", "studio")]
      : []),
    ...(process.env.XDG_CONFIG_HOME
      ? [path.join(process.env.XDG_CONFIG_HOME, "bb", "plugins", "studio")]
      : []),
    ...(process.env.XDG_STATE_HOME
      ? [path.join(process.env.XDG_STATE_HOME, "bb", "plugins", "studio")]
      : []),
  ];
  ...
  try {
    ...
    const expectedStamp = createPluginStudioRuntimeStamp(
      standalone.manifest,
      standalone.manifestBytes,
    );
    assert(
      JSON.stringify(RUNTIME_ARTIFACT_STAMP) === JSON.stringify(expectedStamp),
      "The committed Studio runtime stamp does not match a fresh deterministic standalone build.",
    );
    ...
    const bbPluginStudioSourceRoot = path.join(
      hostileCwd,
      "bb-plugin-studio-source",
    );
    ...
    await Promise.all([
      ...
      ...[bbPluginStudioSourceRoot, gridSourceRoot].map((root) =>
        fs.writeFile(
          path.join(root, "package.json"),
          `${JSON.stringify({
      ...
      writePassiveTarget(
        path.join(bbPluginStudioSourceRoot, "plugins", "linear"),
        "linear",
        "Linear",
        targetMarker,
      ),
      writePassiveTarget(
        path.join(bbPluginStudioSourceRoot, "plugins", "plugin-studio"),
        "plugin-studio",
        "Plugin Studio",
        targetMarker,
      ),
    ...
    assert(
      runtimeResult.stdout.includes("bb-plugin-studio"),
      "Extracted Studio runtime did not identify itself.",
    );
    assert(
      !(await fs
        .access(marker)
    ...
    assert(
      !(await fs
        ...
        .then(() => true)
        .catch(() => false)),
      "Studio lifecycle inspected or executed disposable target plugin code.",
    );
    assert(
    ...
    assert(
      Buffer.compare(originalStamp, await fs.readFile(generatedStampPath)) ===
        0,
      "Studio package proof did not restore the generated runtime stamp.",
    );
    console.log(
      JSON.stringify(
        {
